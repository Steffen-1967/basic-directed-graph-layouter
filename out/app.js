...
import { validateAndTransformGraph, evolveSuccessors, calculateFlowLayout, calculateBoxLayout, calculateTreeLayout, calculateGraphBoundings } from './layouterCalculate.js';
import { render, drawNodeHandles, calculateArrowPath, calculateTreePath, drawEdgeHandles, calculateNodeBoundingBox, calculateAnchorHandles } from './renderer.js';
import { initialAppState, stateEvents } from './state.js';
// DOM Elements
...
const nodeToolboxOverlay = document.getElementById('nodeToolboxOverlay');
const edgeToolboxOverlay = document.getElementById('edgeToolboxOverlay');
const colorPickerOverlay = document.getElementById('colorPickerOverlay');
const edgeWeightOverlay = document.getElementById('edgeWeightOverlay');
...
const recoveryModalMessage = document.getElementById('recoveryModalMessage');
const recoveryModalYesBtn = document.getElementById('recoveryModalYesBtn');
const recoveryModalNoBtn = document.getElementById('recoveryModalNoBtn');
const leavePageModal = document.getElementById('leavePageModal');
const leavePageModalMessage = document.getElementById('leavePageModalMessage');
const leavePageModalYesBtn = document.getElementById('leavePageModalYesBtn');
const leavePageModalNoBtn = document.getElementById('leavePageModalNoBtn');
...
function showToolbox(node, _mouseX, _mouseY) {
...
function showEdgeToolbox(edge, x, y) {
...
function hideEdgeWeightOverlay() {
    edgeWeightOverlay.style.display = 'none';
}
function applyEdgeWeight(edge, newWeight) {
    const startNode = state.nodes.find(n => n.id === edge.fromId);
    const endNode = state.nodes.find(n => n.id === edge.toId);
    if (!startNode || !endNode)
        return;
...
function applyColor(node, fillColor, strokeColor) {
...
window.addEventListener('mousedown', (e) => {
    if (nodeToolboxOverlay.style.display === 'grid' && !nodeToolboxOverlay.contains(e.target)) {
        hideToolbox();
    }
    if (edgeToolboxOverlay.style.display === 'grid' && !edgeToolboxOverlay.contains(e.target)) {
        hideEdgeToolbox();
    }
    if (colorPickerOverlay.style.display === 'grid' && !colorPickerOverlay.contains(e.target)) {
        hideColorPicker();
    }
    if (edgeWeightOverlay.style.display === 'flex' && !edgeWeightOverlay.contains(e.target)) {
        hideEdgeWeightOverlay();
    }
});
...
function requestLockStatus(scenarioName) {
    if (state.network.ws && state.network.ws.readyState === WebSocket.OPEN)
        state.network.ws.send(JSON.stringify({ type: 'lock_status', scenario: scenarioName }));
}
function requestLock(scenarioName) {
    if (state.network.ws && state.network.ws.readyState === WebSocket.OPEN)
        state.network.ws.send(JSON.stringify({ type: 'lock_request', scenario: scenarioName }));
}
function releaseLock(scenarioName) {
    if (state.network.ws && state.network.ws.readyState === WebSocket.OPEN)
        state.network.ws.send(JSON.stringify({ type: 'lock_release', scenario: scenarioName }));
}
...
function showLeavePageModal(message) {
    return new Promise((resolve) => {
...
async function saveData() {
    if (!state.scenario || !state.interaction.isEditable)
        return;
    try {
    ...
    if (state.interaction.isEditable) {
        ...
        if (foundEdgeAnchor === null) {
            for (const node of state.nodes) {
                ...
                if (node.type === 'Event') {
                    ...
                    if (worldX >= node.x - w && worldX <= node.x + w && worldY >= node.y - h && worldY <= node.y + h)
                        hit = true;
                }
                else if (node.type === 'Rule') {
                    if (Math.abs(worldX - node.x) + Math.abs(worldY - node.y) <= CONFIG.sizes.ruleSize * 0.5 + hoverExpansion)
                        hit = true;
                }
                if (hit)
                    foundNode = node;
        ...
        for (const node of state.nodes) {
            ...
            if (node.type === 'Event') {
                ...
                if (Math.abs(worldX - node.x) + Math.abs(worldY - node.y) <= CONFIG.sizes.ruleSize * 0.5)
                    hit = true;
            }
            if (hit)
                foundNode = node;
...
window.addEventListener('beforeunload', (e) => { if (history.isDirty()) {
    e.preventDefault();
    e.returnValue = '';
} });
function saveInPlaceEditOnName() {
...
function startNodeNameEdit(node) {
...
async function loadData(fileName) {
    ...
    if (state.network.currentScenarioLock)
        releaseLock(state.network.currentScenarioLock.scenario);
    state.interaction.isEditable = false;
    updateEditButton();
    ...
    try {
        ...
        apiLogger.error('Error loading scenario');
    }
    updateCanvasSize();
    centerGraph();
    requestLockStatus(fileName);
...
undoBtn.addEventListener('click', () => { if (state.scenario) {
    history.undo(state.scenario);
    renderAll();
} });
redoBtn.addEventListener('click', () => { if (state.scenario) {
    history.redo(state.scenario);
    renderAll();
} });
...
async function loadScenariosFromServer() {
    ...
    if (data.success) {
        select.innerHTML = '';
        data.scenarios.forEach((s) => { const o = document.createElement('option'); o.value = s; o.textContent = s; select.appendChild(o); });
        if (data.scenarios.length > 0)
            loadData(data.scenarios[0]);
    }
}
canvas.addEventListener('mousedown', (e) => {
    ...
    state.interaction.startX = e.clientX - state.view.offsetX;
    state.interaction.startY = e.clientY - state.view.offsetY;
});
...
window.addEventListener('mousemove', (e) => {
    const rect = canvas.getBoundingClientRect();
    state.interaction.lastMouseX = e.clientX - rect.left;
    state.interaction.lastMouseY = e.clientY - rect.top;
    if (state.interaction.isDragging) {
        state.view.offsetX = e.clientX - state.interaction.startX;
        state.view.offsetY = e.clientY - state.interaction.startY;
        renderAll();
    }
    else
        updateHoverState(state.interaction.lastMouseX, state.interaction.lastMouseY);
